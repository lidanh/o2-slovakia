...
export async function GET() {
  try {
    ...
    // Top performers
    let usersDataQuery = supabase.from("users").select("id, name, team:teams(name)");
    if (scopedUserIds) {
      usersDataQuery = usersDataQuery.in("id", scopedUserIds);
    }
    const { data: users } = await usersDataQuery;
